...
async function main() {
  ...
  async function ensureBranchCards(species, shaped) {
    ...
    try {
      ...
      console.error('[Tohum Three] branch card bake failed:', e);
      cards = null;
    } finally {
      baking = false;
    }
  ...
  async function rebuild(onStage = null, deferBake = false) {
    ...
    try {
      ...
      if (speciesChanged) {
        ...
        await onStage?.('Koruluk dikiliyor', 0.58);
        updateForest(group);
  ...
  function updateStats(species, group) {
    ...
    statsText =
      ...
      `tohum ${state.controls.seed}  ·  altyapı ${backend}  ·  ${u.stemCount} dal · ${u.leafInstances} yaprak\n` +
      lodLine;
    refreshHud();
  }
  ...
  bakeHud.innerHTML = '<span class="bake-label">Billboard pişiriliyor...</span><span class="bake-track"><i class="bake-fill" id="bake-fill"></i></span>';
  ...
  async function bakeBillboard() {
    ...
    try {
      ...
      console.error('[Tohum Three] billboard bake failed:', e);
    } finally {
      baking = false;
  ...
  async function heavyRebuild(label, before = null) {
    ...
    try {
      ...
      if (before) { await stageStep('Biyom kuruluyor', 0.12); await before(); } // swap sky/ground behind the screen
      ...
      await stageStep('Shaderlar derleniyor', 0.66);   // the big one — the pipeline compile blocks on the next render
      ...
      await stageStep('Uzak görünüm pişiriliyor', 0.86);
      ...
      console.error('[Tohum Three] heavy rebuild failed:', e);
    } finally {
      hideLoading();
    }
  }
  ...
  const { syncFromState, applyPreset } = buildGUI({
    ...
    onLoadRebuild: () => heavyRebuild(`${SPECIES[state.speciesKey].name} yükleniyor...`, () => buildBiome(SPECIES[state.speciesKey])),
    ...
    onSpom: async () => {
      ...
      setStage(envState.spom ? 'Paralaks arazi açılıyor...' : 'Paralaks arazi kapatılıyor...', 0.3);
      showLoading();
      try {
        await nextPaint();
    ...
    onChange: (speciesChanged) => {
      if (speciesChanged) {
        ...
        heavyRebuild(`${SPECIES[state.speciesKey].name} büyüyor...`, () => buildBiome(SPECIES[state.speciesKey]));
      } else {
    ...
    onExportPNG: async () => {
      try {
        ...
        a.download = `${SPECIES[state.speciesKey].name.replace(/\s+/g, '_')}_tohum${state.controls.seed}.png`;
    ...
    onExport: async () => {
      ...
      try {
        ...
        const size = await downloadGLB(currentTree, `${SPECIES[state.speciesKey].name.replace(/\s+/g, '_')}_tohum${state.controls.seed}`);
        if (size) console.log(`[Tohum Three] GLB dışa aktarıldı (${(size / 1024).toFixed(0)} KB)`);
      } catch (e) {
        console.error('GLB dışa aktarma başarısız:', e);
        fail(`GLB dışa aktarma başarısız: ${e.message}`);
      }
    },
  });

  // First build compiles pipelines from cold → same overlay treatment as a switch.
  await heavyRebuild('Tohum Three büyüyor...', async () => {
    await buildBiome(SPECIES[state.speciesKey]);
