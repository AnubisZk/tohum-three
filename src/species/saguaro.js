...
export const saguaro = {
  ...
  controls: [
    { key: 'trunkHeight', name: 'Sütun büyümesi (m/seg)', min: 0.7, max: 2.2, step: 0.1, get: (s) => s.params.firstForkHeight, set: (s, v) => { s.params.firstForkHeight = v; } },
    { key: 'armLength', name: 'Kol uzunluğu (m)', min: 0.6, max: 2, step: 0.1, get: (s) => s.params.armLength, set: (s, v) => { s.params.armLength = v; } },
    { key: 'forkGenerations', name: 'Sütun segmentleri', min: 3, max: 8, step: 1, get: (s) => s.params.forkGenerations, set: (s, v) => { s.params.forkGenerations = Math.round(v); } },
    { key: 'branchiness', name: 'Kol sıklığı', min: 0.0, max: 0.7, step: 0.05, get: (s) => s.params.branchiness, set: (s, v) => { s.params.branchiness = v; } },
    { key: 'forkSpread', name: 'Kol açılımı (°)', min: 40, max: 90, step: 2, get: (s) => s.params.forkSpread, set: (s, v) => { s.params.forkSpread = v; } },
    { key: 'curlUp', name: 'Kol yukarı kıvrımı', min: 0.2, max: 0.85, step: 0.05, get: (s) => s.params.curlUp, set: (s, v) => { s.params.curlUp = v; } },
    ...
    { key: 'ribCount', name: 'Kaburga sayısı', min: 12, max: 24, step: 4, get: (s) => s.params.ribCount, set: (s, v) => { s.params.ribCount = Math.round(v / 4) * 4; s.params.radialSegs = Math.max(48, s.params.ribCount * 4); } },
    { key: 'ribDepth', name: 'Kaburga derinliği', min: 0.04, max: 0.2, step: 0.01, get: (s) => s.params.ribDepth, set: (s, v) => { s.params.ribDepth = v; } },
    { key: 'trunkThickness', name: 'Sütun kalınlığı', min: 0.5, max: 2, step: 0.05, get: () => 1, set: (s, v) => { s.params.trunkRadius *= v; } },
    { key: 'spineDensity', name: 'Diken yoğunluğu', min: 0.15, max: 1, step: 0.05, get: (s) => s.spines?.density ?? 1, set: (s, v) => { s.spines.density = v; } },
  ],
  // Advanced L-system dials — the candelabra arm system + column shaping the
  // Shape panel doesn't expose.
  advancedControls: [
    { key: 'armAsymmetric', name: 'Kol stili', dropdown: { 'Şamdan (asimetrik)': true, 'Simetrik V': false }, get: (s) => s.params.armAsymmetric ?? true, set: (s, v) => { s.params.armAsymmetric = v; } },
    { key: 'armMinHeightFrac', name: 'Kol min. yüksekliği', min: 0, max: 0.8, step: 0.05, get: (s) => s.params.armMinHeightFrac ?? 0.2, set: (s, v) => { s.params.armMinHeightFrac = v; } },
    { key: 'armMaxOrder', name: 'Kol maks. derecesi', min: 1, max: 4, step: 1, get: (s) => s.params.armMaxOrder ?? 1, set: (s, v) => { s.params.armMaxOrder = Math.round(v); } },
    { key: 'armGenerations', name: 'Kol uzunluğu (segment)', min: 0, max: 8, step: 1, get: (s) => s.params.armGenerations ?? 5, set: (s, v) => { s.params.armGenerations = Math.round(v); } },
    { key: 'armFalloff', name: 'Kol azalımı / nesil', min: 0.5, max: 1, step: 0.02, get: (s) => s.params.armFalloff ?? 0.9, set: (s, v) => { s.params.armFalloff = v; } },
    { key: 'forkTriChance', name: '2 kol şansı', min: 0, max: 0.6, step: 0.05, get: (s) => s.params.forkTriChance ?? 0.15, set: (s, v) => { s.params.forkTriChance = v; } },
    { key: 'forkRadiusKeep', name: 'Kol kalınlığı koru', min: 0.5, max: 1, step: 0.02, get: (s) => s.params.forkRadiusKeep ?? 0.72, set: (s, v) => { s.params.forkRadiusKeep = v; } },
    { key: 'forkBaseScale', name: 'Kol tabanı boynu', min: 0.4, max: 1.2, step: 0.02, get: (s) => s.params.forkBaseScale ?? 0.58, set: (s, v) => { s.params.forkBaseScale = v; } },
    { key: 'branchRepel', name: 'Dal itimi', min: 0, max: 1.5, step: 0.05, get: (s) => s.params.branchRepel ?? 0.6, set: (s, v) => { s.params.branchRepel = v; } },
    { key: 'trunkSegRes', name: 'Sütun halka detayı', min: 3, max: 16, step: 1, get: (s) => s.params.trunkSegRes ?? 9, set: (s, v) => { s.params.trunkSegRes = Math.round(v); } },
  ],
