...
export const CROWN_SHAPES = {
  Konik: 0, Kuresel: 1, 'Yarim kure': 2, Silindirik: 3,
  'Daralan silindir': 4, Alev: 5, 'Ters konik': 6, 'Alev egilimli': 7,
};
...
export const ADVANCED_LEVEL_PARAMS = [
  { key: 'downAngle',      name: 'Asagi aci',   min: 0,    max: 135, step: 1,    trunk: false, dflt: 0 },
  { key: 'branches',       name: 'Cocuk dal',   min: 0,    max: 60,  step: 1,    trunk: false, dflt: 0 },
  { key: 'curveV',         name: 'Burukluk',    min: 0,    max: 120, step: 1,    trunk: true,  dflt: 40 },
  { key: 'curve',          name: 'Egri',        min: -90,  max: 90,  step: 1,    trunk: true,  dflt: 0 },
  { key: 'length',         name: 'Uzunluk ×',   min: 0.02, max: 1.5, step: 0.01, trunk: false, dflt: 0.4 },
  { key: 'taper',          name: 'Incelme',     min: 0,    max: 1,   step: 0.01, trunk: true,  dflt: 1 },
  { key: 'twist',          name: 'Burulma',     min: -0.5, max: 0.5, step: 0.01, trunk: true,  dflt: 0 },
  { key: 'curveRes',       name: 'Kesitler',    min: 2,    max: 20,  step: 1,    trunk: true,  dflt: 8 },
  { key: 'radialSegments', name: 'Segmentler',  min: 3,    max: 16,  step: 1,    trunk: true,  dflt: 6 },
];
...
export function buildGUI(opts) {
  ...
  brand.innerHTML = `
    ...
    <span class="wordmark-text">Tohum Three</span>`;
  ...
  function applyMobileUI() {
    ...
    cMeshQ.name(m ? 'Dal / iskelet kalitesi' : 'LOD0 mesh kalitesi');
    cLod1Den.name(m ? 'LOD1 kart yogunlugu' : 'LOD1 yaprak yogunlugu');
    cLod2Den.name(m ? 'LOD2 kart yogunlugu' : 'LOD2 yaprak yogunlugu');
    cLod1Prn.name(m ? 'LOD1 dalcik budama' : 'LOD1 dal budama');
    cLod2Prn.name(m ? 'LOD2 dalcik budama' : 'LOD2 dal budama');
  }

  gui.add(proxy, 'species', speciesNames).name('Tur').onChange((key) => {
  ...
  gui.add(proxy, 'seed', 1, 9999, 1).name('Tohum').onChange((v) => { state.controls.seed = v; onChange(); }).listen();
  gui.add({ randomize: () => onRandomize() }, 'randomize').name('🎲 Rastgele tohum');

  // Species-defined controls: rebuilt whenever the species changes so each
  // plant exposes sliders for ITS OWN branching type.
  const shape = gui.addFolder('Form ve Yaprak');
  function buildParamControls() {
    ...
    shape.add(proxy, 'showLeaves').name('Yapraklari goster').onChange((v) => { state.controls.showLeaves = v; onChange(); });
    shape.add(proxy, 'tileWorldSize', 0.6, 3.0, 0.05).name('Kabuk dokusu (m)').onChange((v) => { state.controls.tileWorldSize = v; onChange(); });
  }
  buildParamControls();
  ...
  // neither.
  const advanced = gui.addFolder('Gelismis: dal seviyeleri');
  function buildAdvancedControls() {
    ...
    if (advanced.title) advanced.title(isRosette ? 'Gelismis: L-sistemi' : 'Gelismis: dal seviyeleri');
    if (isRosette) {
    ...
    for (const axis of ['X', 'Y', 'Z']) {
      ...
      advanced.add(proxy, pk, -1, 1, 0.01).name(`Kuvvet yonu ${axis}`).onChange((v) => { state.controls[pk] = v; onChange(); });
    }
    advanced.add(proxy, 'forceStrength', 0, 0.12, 0.001).name('Kuvvet siddeti').onChange((v) => { state.controls.forceStrength = v; onChange(); });
  }
  buildAdvancedControls();
  ...
  const leaves = gui.addFolder('Yapraklar');
  const fronds = gui.addFolder('Rozet yapraklar'); // rosette foliage (Joshua/yuccas)
  const spines = gui.addFolder('Dikenler'); // cactus (saguaro)
  const bark = gui.addFolder('Kabuk');
  function buildLeafBarkControls() {
    ...
    if (!isRosette) {
      ...
      leaves.addColor(proxy, 'leafColorize').name('Renk').onChange(mtweak('leafColorize'));
      leaves.add(proxy, 'leafTintAmount', 0, 1, 0.01).name('Renk miktari').onChange(mtweak('leafTintAmount'));
      leaves.add(proxy, 'leafAngle', 0, 100, 1).name('Aci').onChange(geom('leafAngle'));
      leaves.add(proxy, 'leafStart', 0, 1, 0.01).name('Baslangic').onChange(geom('leafStart'));
      leaves.add(proxy, 'leafSizeVar', 0, 1, 0.01).name('Boyut cesitliligi').onChange(geom('leafSizeVar'));
      leaves.add(proxy, 'leafAlpha', 0, 1, 0.01).name('Alfa esigi').onChange(mtweak('leafAlpha'));
      leaves.add(proxy, 'leafQuads', { 'Tek': 1, 'Capraz (cift)': 2 }).name('Billboard').onChange(geom('leafQuads'));
    }
    ...
    if (isFrondRosette) {
      fronds.addColor(proxy, 'frondGreenTint').name('Yesil renk').onChange(mtweak('frondGreenTint'));
      fronds.addColor(proxy, 'frondDryTint').name('Kuru renk').onChange(mtweak('frondDryTint'));
      fronds.addColor(proxy, 'frondDryestTint').name('En kuru renk').onChange(mtweak('frondDryestTint'));
      fronds.add(proxy, 'frondDryness', 0, 1, 0.01).name('Kuruluk').onChange(mtweak('frondDryness'));
    }
    // Spines (saguaro): simple tint over the spine albedo.
    spines.domElement.style.display = isCactus ? '' : 'none';
    if (isCactus) {
      spines.addColor(proxy, 'spineTint').name('Diken rengi').onChange(mtweak('spineTint'));
    }
    bark.addColor(proxy, 'barkTint').name('Renk').onChange(mtweak('barkTint'));
    bark.add(proxy, 'barkFlat').name('Duz golgeleme').onChange(mtweak('barkFlat'));
    ...
    if (isCactus) bark.add(proxy, 'fleshDamage', 0, 1, 0.01).name('Gövde yipranmasi').onChange(mtweak('fleshDamage'));
  }
  buildLeafBarkControls();

  // Optimization: LOD chain preview + switch distances + billboard bake options.
  if (optState && onOpt) {
    const opt = gui.addFolder('Optimizasyon / LOD');
    opt.add(optState, 'preview', {
      'Otomatik (mesafeye gore)': 'auto',
      'LOD0 - tam detay': 0,
      'LOD1 - azaltilmis geometri': 1,
      'LOD2 - pisirilmis kartlar': 2,
      'LOD3 - billboard': 3,
    }).name('Onizleme seviyesi').onChange(() => onOpt('preview'));
    ...
    cMobile = opt.add(optState, 'mobileTarget').name('Mobil performans hedefi')
      .onChange(() => { applyMobileUI(); onOpt('rebuild'); });
    cMeshQ = opt.add(optState, 'meshQuality', 0.3, 1, 0.05).name('LOD0 mesh kalitesi').onChange(() => onOpt('rebuild'));
    opt.add(optState, 'lod1Dist', 5, 80, 1).name('LOD1 mesafesi (m)').onChange(() => onOpt('dist'));
    opt.add(optState, 'lod2Dist', 15, 150, 1).name('LOD2 mesafesi (m)').onChange(() => onOpt('dist'));
    opt.add(optState, 'billboardDist', 30, 300, 1).name('Billboard mesafesi (m)').onChange(() => onOpt('dist'));
    ...
    cLod1Pct = opt.add(optState, 'lod1Pct', 15, 85, 5).name('LOD1 butce (%)').onChange(() => onOpt('rebuild'));
    cLod1Den = opt.add(optState, 'lod1Density', 0.3, 1, 0.05).name('LOD1 yaprak yogunlugu').onChange(() => onOpt('rebuild'));
    cLod1Prn = opt.add(optState, 'lod1Prune', 0, 0.85, 0.05).name('LOD1 dal budama').onChange(() => onOpt('rebuild'));
    cLod2Pct = opt.add(optState, 'lod2Pct', 4, 40, 1).name('LOD2 butce (%)').onChange(() => onOpt('rebuild'));
    cLod2Den = opt.add(optState, 'lod2Density', 0.2, 1, 0.05).name('LOD2 yaprak yogunlugu').onChange(() => onOpt('rebuild'));
    cLod2Prn = opt.add(optState, 'lod2Prune', 0, 0.85, 0.05).name('LOD2 dal budama').onChange(() => onOpt('rebuild'));
    ...
    opt.add(optState, 'cardRes', { '256²': 256, '512²': 512, '1024²': 1024 }).name('Kart pisirme coz.').onChange(() => onOpt('rebuild'));
    opt.add(optState, 'cardVariants', { 2: 2, 3: 3, 4: 4 }).name('Kart varyanti').onChange(() => onOpt('rebuild'));
    opt.add(optState, 'billboardRes', { '512²': 512, '1024²': 1024, '2048²': 2048 }).name('Billboard coz.').onChange(() => onOpt('rebake'));
  }

  if (sunState && onSun) {
    const env = gui.addFolder('Cevre');
    env.add(sunState, 'az', 0, 360, 1).name('Gunes azimutu').onChange(() => onSun());
    env.add(sunState, 'el', 5, 88, 1).name('Gunes yuksekligi').onChange(() => onSun());
    if (windState && onWind) {
      env.add(windState, 'strength', 0, 1, 0.05).name('Ruzgar siddeti').onChange(() => onWind());
      env.add(windState, 'speed', 0.2, 2.5, 0.05).name('Ruzgar hizi').onChange(() => onWind());
    }
    if (envState && onScaleRef) {
      env.add(envState, 'showScaleRef').name('Olcek referansi (1.8m)').onChange((v) => onScaleRef(v));
      if (onFog) env.add(envState, 'fog').name('Uzaklik sisi').onChange(() => onFog());
      if (onSpom) env.add(envState, 'spom').name('Paralaks arazi (SPOM)').onChange(() => onSpom());
      if (onGtao) env.add(envState, 'gtao').name('Ortam golgesi (GTAO)').onChange(() => onGtao());
      if (onAA) env.add(envState, 'aa').name('Kenar yumusatma (MSAA)').onChange(() => onAA());
      if (onForest) env.add(envState, 'forestCount', 0, 96, 8).name('Orman agaclari').onChange(() => onForest());
    }
  }
  ...
  if (camState && onCamera) {
    ...
    cam.add(camState, 'autoRotate').name('Otomatik donus').onChange(() => onCamera());
    cam.add(camState, 'autoRotateSpeed', 0, 4, 0.1).name('Donus hizi').onChange(() => onCamera());
  }

  gui.add({ export: () => onExport() }, 'export').name('⬇ .glb indir');
  if (onExportPNG) gui.add({ png: () => onExportPNG() }, 'png').name('📷 PNG disa aktar');
  ...
  const savePreset = () => {
    const preset = { format: 'tohum-three-preset/1', species: state.speciesKey, controls: state.controls };
    const blob = new Blob([JSON.stringify(preset, null, 2)], { type: 'application/json' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `${speciesMap[state.speciesKey].name.replace(/\s+/g, '_')}_tohum${state.controls.seed}.tohum-three.json`;
  ...
  const io = gui.addFolder('Kaydet ve Yukle');
  io.add({ save: () => savePreset() }, 'save').name('💾 Preset kaydet');
  io.add({ load: () => loadPreset() }, 'load').name('📂 Preset yukle');
